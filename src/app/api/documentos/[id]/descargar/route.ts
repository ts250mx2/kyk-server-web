...
// Vista previa (o descarga, solo oficina) de un documento con auditoría:
// valida el acceso de la tienda, registra el acceso y regresa el archivo.
// ?vista=1 lo sirve inline para el navegador; las tiendas SOLO pueden
// visualizar — la descarga como archivo queda reservada al rol oficina.
    ...
    try {
        ...
        if (!enLinea && !oficina) {
            return NextResponse.json(
                { error: 'Los documentos solo se pueden visualizar' },
                { status: 403 }
            );
        }

        const docs = await portalQuery(`
            SELECT IdDocumento, Nombre, NombreArchivo, Archivo, Contenido, TipoMime, TodasTiendas, Status
        ...
        if (num(doc.TodasTiendas) !== 1 && !oficina) {
        ...
        // El binario vive en la base; los documentos previos a la migración
        // se leen del disco como respaldo
        const contenido = Buffer.isBuffer(doc.Contenido) && doc.Contenido.length > 0
            ? doc.Contenido
            : await leerArchivo(str(doc.Archivo));
